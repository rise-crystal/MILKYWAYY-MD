...
import { jidDecode, areJidsSameUser, getAggregateVotesInPollMessage, downloadContentFromMessage } from "@whiskeysockets/baileys";
...
class Sockets {
  ...
  async reply(jid, text, quoted, options) {
    return this.sendText(jid, text, quoted, options)
  }
  ...
  async sendList(jid = '', text = '', footer = '', buttonText = '', title = '', rows = [], quoted = '') {
    const list = {
      text,
      footer,
      buttonText,
    ...
    return this.sendMessage(jid, list, { quoted })
  }
  ...
  async downloadMediaMessage(message, MessageType) {
