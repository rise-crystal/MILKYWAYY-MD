  ...
  constructor (message, conn) {
    ...
    if (this.message) {
      ...
      try {
        ...
        this.quoted.mtype = this.quoted.type;
      ...
      '';
      if (!db.config.prefix) { db.config.prefix = {}; }
      if (!db.config.prefix.multiPrefx) { db.config.prefix.multiPrefx = true; }
      if (!db.config.prefix.noPrefix) { db.config.prefix.noPrefix = ''; }
      if (!db.config.prefix.noPrefix) { db.config.prefix.singlePrefix = '#'; }
     
      if (db.config.prefix.multiPrefx === true) {
        this.prefix = /^[°•π÷×¶∆£¢€¥®™✓_=|~!?#$%^&.+-,\/\\©^]/.test(this.body) ? this.body.match(/^[°•π÷×¶∆£¢€¥®™✓_=|~!?#$%^&.+-,\/\\©^]/gi) : db.config.prefix.singlePrefix;
      } else if (db.config.prefix.multiPrefx === false) {
      ...
      try {
        ...
        this.args = this.body.toLowerCase().split(/ +/).slice(1);
      }
